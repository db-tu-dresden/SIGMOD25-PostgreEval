...
def try_exhaustive_enumeration(
    query: qal.SqlQuery, *, query_label: str, n_plans: int
    ...
    report = SamplingReport(
        ...
        query=query,
        base_table=None,
        plan_structure="bushy",
        cost_area="arbitrary",
        sampling_duration=enumeration_duration,
        n_requested=n_plans,
        n_produced=n_generated,
        generator="exhaustive",
    )
    return SamplingResult(plans=set(plans), reports=[report])


def initialize_plan_sampler(
    plan_structure: PlanStructure,
) -> randomized.RandomJoinOrderGenerator:
    return randomized.RandomJoinOrderGenerator(
        eliminate_duplicates=False, tree_structure=plan_structure
    )


def sample_uniform_base_table_plans(
    generator: randomized.RandomJoinOrderGenerator,
    query: qal.SqlQuery,
    *,
    plan_structure: PlanStructure,
    query_label: str,
    ...
    for cost_area, base_table in itertools.product(cost_areas, base_tables):
        current_result = sample_cost_based(
            generator.random_join_orders_for(query, base_table=base_table),
            query,
            cost_area=cost_area,
            base_table=base_table,
            plan_structure=plan_structure,
            query_label=query_label,
            n_plans=plans_per_table,
...
def sample_cost_based(
    generator: Generator[jointree.LogicalJoinTree, None, None],
    query: qal.SqlQuery,
    *,
    cost_area: CostArea,
    base_table: base.TableReference,
    plan_structure: PlanStructure,
    n_plans: int,
    retry_factor: float = 5.0,
    card_provider: cardinalities.PreComputedCardinalities,
    ...
    physical_plan = jointree.PhysicalQueryPlan.load_from_query_plan(
        native_query_plan, query, operators_only=True
    )
    actual_cardinalities = card_provider.generate_plan_parameters(
        query, physical_plan.as_logical_join_tree(), None
    )
    actual_cardinality_hinted_query = pg_instance.hinting().generate_hints(
        query, physical_plan, plan_parameters=actual_cardinalities
    )

    native_plan_cost = pg_instance.optimizer().cost_estimate(
        actual_cardinality_hinted_query
    )
    ...
    while len(sampled_plans) < n_plans and n_tries < max_tries:
        ...
        hinted_query = pg_instance.hinting().generate_hints(
            query, physical_plan, plan_parameters=actual_cardinalities
        )
        ...
        close_match = (
            cost_area == "close"
            and 0.7 * native_plan_cost <= current_cost <= 1.3 * native_plan_cost
        )
    ...
    report = SamplingReport(
        ...
        plan_structure=plan_structure,
        cost_area=cost_area,
        generator="sampled_guided",
        sampling_duration=sampling_duration,
        n_requested=n_plans,
        n_produced=len(sampled_plans),
    )
...
def sample_uniform(
    ...
    *,
    n_plans: int,
    retry_factor: float = 5.0,
    existing_plans: set[jointree.LogicalJoinTree],
) -> SamplingResult:
    all_sampled_plans = set(existing_plans)
    plan_generator = randomized.RandomJoinOrderGenerator(
        eliminate_duplicates=False, tree_structure="bushy"
...
def execute_query(
    label: str,
    join_order: jointree.LogicalJoinTree,
    *,
    ...
    true_cardinalities = cardinalitiy_provider.generate_plan_parameters(
        query, join_order, None
    )
    hinted_query = database.hinting().generate_hints(
        query, join_order, plan_parameters=true_cardinalities
    )
...
def skip_existing_results(
    out_dir: str, *, benchmark: workloads.Workload
) -> workloads.Workload[str]:
...
def flush_sampling_reports(reports: Sequence[SamplingReport], out_dir: str) -> None:
    ...
    existing_reports = (
        pd.read_csv(target_path) if target_path.exists() else pd.DataFrame()
    )
...
def benchmark_missing_queries(
    out_dir: str,
    *,
    benchmark: workloads.Workload,
    pg_instance: postgres.PostgresInterface,
    n_plans: int,
    linear_fraction: float = 0.7,
    retry_factor: float = 5.0,
    min_timeout: float = 15.0,
    max_timeout: Optional[float],
    native_runtimes_file: str,
    cardinalities_file: str,
    forced_queries: Optional[set[str]] = None,
) -> None:
    signal.signal(signal.SIGINT, ctl_c_handler)
    true_card_generator = cardinalities.PreComputedCardinalities(
        benchmark, cardinalities_file, error_on_missing_card=False
    )
    if forced_queries:
    ...
    for label, query in remaining_workload.entries():
        ...
        current_timeout = determine_timeout(
            label,
        ...
        exhaustive_generation = try_exhaustive_enumeration(
            query, query_label=label, n_plans=n_plans
        )
        if exhaustive_generation is not None:
            ...
            sampled_plans, sampling_reports = sample_uniform_base_table_plans(
                linear_plan_generator,
                query,
                plan_structure="left-deep",
                query_label=label,
...
def make_explain_parser(
    label: str, *, benchmark: workloads.Workload
) -> Callable[[str], jointree.LogicalJoinTree]:
    def loader(raw_plan: str) -> jointree.LogicalJoinTree:
        ...
        return jointree.LogicalJoinTree.load_from_query_plan(
            pg_plan.as_query_execution_plan(), benchmark[label]
        )

    return loader


def fill_missed_samples(
    out_dir: str,
    *,
    benchmark: workloads.Workload,
    pg_instance: postgres.PostgresInterface,
    ...
    for label, query in benchmark.entries():
        ...
        existing_results["query_plan"] = existing_results["query_plan"].apply(
            make_explain_parser(label, benchmark=benchmark)
        )
        ...
        current_samples, sampling_report = sample_uniform(
            label,
            query,
            n_plans=n_missing_samples,
            retry_factor=retry_factor,
...
def main() -> None:
    ...
    parser.add_argument(
        "--native-rts", type=str, default="results/job/job-native-runtimes.csv"
    )
    parser.add_argument(
        "--cardinalities",
        type=str,
        default="results/job/job-intermediate-cardinalities.csv",
    )
    ...
    if args.fill_samples:
        fill_missed_samples(
            ...
            benchmark=benchmark,
            pg_instance=pg_instance,
            n_target_plans=args.n_plans,
            native_runtimes_file=args.native_rts,
